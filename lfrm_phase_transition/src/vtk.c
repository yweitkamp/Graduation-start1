...
void vtkPrepareRectilinearMesh_FT_ENERGY(char *fname)
{
  ...
  int nvars = 6;
  int vardims[] = {1,1,3,1,1,1};
  int centering[] = {1,1,1,1,1,1};
  const char *varnames[] = {"VOF", "Pressure", "Velocity", "T", "Rhocp","massflux"};
  ...
  float* vec_cont     = malloc((nx)*(ny)*(nz)*sizeof(float[3]));
  float* rhocp 		  = malloc((nx)*(ny)*(nz)*sizeof(float));
  float* massflux 	  = malloc((nx)*(ny)*(nz)*sizeof(float));

  float* xcoords      = malloc(nx*sizeof(float));
  float* ycoords      = malloc(ny*sizeof(float));
  float* zcoords      = malloc(nz*sizeof(float));

  float *vars[] = {(float *)dispersefrac, (float*) pressure, (float*)vec_cont, (float*)Temperature, (float*)rhocp, (float*)massflux};
  ...
  for(k=1;k<=nz;k++) for(j=1;j<=ny;j++) for(i=1;i<=nx;i++)
      {
          ...
          rhocp[p]         =  (float) mac_rhoCp[i][j][k];

          massflux[p]      =  (float) mmm[i][j][k];
