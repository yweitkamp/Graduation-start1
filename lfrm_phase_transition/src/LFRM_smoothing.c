...
void LFRM_DETERMINE_BALL_OF_MARKERS(int bnr, int newpoint, struct LFRM *LFRM, int *newtooldpoint, int *oldtonewmarker)
{
	/* Function to create ballmrks after interface reconstruction, ballmrks is NOT in an (anti-)clockwise order */
	int i, oldpoint;

	oldpoint = newtooldpoint[newpoint];

	for (i = 0; i < LFRM->pointtotrianglecount[oldpoint]; i++)
	{
		ballmrks[bnr][newpoint][i] = oldtonewmarker[ LFRM->pointtotriangle[oldpoint][i] ];
	}
...
void LFRM_DETERMINE_BALL_OF_ALL_VERTICES(int bnr, struct LFRM *LFRM, int **tempmar, int *newtooldpoint, int *oldtonewpoint, int *oldtonewmarker)
{
	...
	for (nnp = 0; nnp < npos[bnr]; nnp++)
	{
		LFRM_DETERMINE_BALL_OF_MARKERS(bnr, nnp, LFRM, newtooldpoint, oldtonewmarker);
