...
void LFRM_RENUMBERING(int bnr, struct region bubblereg, struct LFRM *LFRM, double **temppos, int **tempmar, int numpos, int nummar)
{
	...
	int *oldtonewpoint, *newtooldpoint, *oldtonewmarker;
	...
	oldtonewmarker = inte_1D_array(nummar);
	...
	LFRM_DETERMINE_BALL_OF_ALL_VERTICES(bnr, LFRM, tempmar, newtooldpoint, oldtonewpoint, oldtonewmarker);

/*
	if (bnr == 0)
	{
		printf("On bubble: %d\n", bnr);
		printf("Vertices around %d are: ", midpoint);
		for ( i = 0; i < ballcnt[bnr][midpoint]; i++)
		{
			printf("%d, ", ballpnts[bnr][midpoint][i]);
		}
		printf("\n");
		for ( i = 0; i < ballcnt[bnr][midpoint]; i++)
		{
			printf("Marker: %d has vertices: %d, %d, %d \n", ballmrks[bnr][midpoint][i], markpos[bnr][ ballmrks[bnr][midpoint][i] ][0], markpos[bnr][ ballmrks[bnr][midpoint][i] ][1], markpos[bnr][ ballmrks[bnr][midpoint][i] ][2]);
		}
	}
*/
